...
namespace ICSharpCode.SharpZipLib.BZip2
{
	...
	public class BZip2BlockDecompressor {
		 ...
		 */
		private byte[] huffmanSymbolMap = new byte[256];
		 ...
		 */
		private int[] bwtByteCounts = new int[256];
		 ...
		 * stage (before Run-Length Decoding)
		 */
		...
		private BZip2HuffmanStageDecoder readHuffmanTables() {
			...
			var tableCodeLengths = new byte[BZip2Constants.HUFFMAN_MAXIMUM_TABLES, BZip2Constants.HUFFMAN_MAXIMUM_ALPHABET_SIZE];
		...
		private void decodeHuffmanData(BZip2HuffmanStageDecoder huffmanDecoder) {
			...
			for (; ; ) {
				...
				if (nextSymbol == BZip2Constants.HUFFMAN_SYMBOL_RUNA) {
					...
					if (repeatCount > 0) {
						if (_bwtBlockLength + repeatCount > streamBlockSize) {
					...
					if (_bwtBlockLength >= streamBlockSize) {
						throw new BZip2Exception("BZip2 block exceeds declared block size");
					}
		...
		private void initialiseInverseBWT(uint bwtStartPointer) {
			...
			bwtMergedPointers = new int[bwtBlockLength];
		 ...
		 */
		private byte decodeNextBWTByte()
		{

			byte nextDecodedByte = (byte)bwtCurrentMergedPointer;
			bwtCurrentMergedPointer = bwtMergedPointers[bwtCurrentMergedPointer >> 8];

			if (blockRandomised && --randomCount == 0)
			{
				nextDecodedByte ^= 1;
		...
		/**
		 * Decodes a byte from the Run-Length Encoding stage, pulling a new byte from the
		...
		public int Read() {
			...
			while (this.rleRepeat < 1) {
				...
				byte nextByte = decodeNextBWTByte();
		...
		/**
		 * Decodes multiple bytes from the Run-Length Encoding stage, pulling new bytes from the
		 * Burrows-Wheeler Transform stage when required
		...
		public uint CheckCRC()
		{
			...
			if (blockCRC != crc.CRC) {
				throw new BZip2Exception($"BZip2 block CRC error, should be:\n0x{blockCRC:x8}\nbut is instead\n0x{crc.CRC:x8}");
			}
		...
		public BZip2BlockDecompressor(BZip2BitInputStream bitInputStream, uint blockSize) {
			...
			// Read block header
			blockCRC = this.bitInputStream.readInteger();
			blockRandomised = this.bitInputStream.readBoolean();
